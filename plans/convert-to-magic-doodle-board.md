...
- Observation: the Windows zlib-ng and libpng depot archives use the dynamic MSVC runtime, while the published Windows Skia archive uses the static runtime.
  Evidence: after the Skia `/MT` correction, hosted Windows run `29542221102` linked Skia but reported `__imp_*` C-runtime symbols from `libpng16_static.lib` and `zlibstatic.lib`; it also exposed the desktop demo's missing C++17 target requirement. Run `29542455555` then showed that a dynamic vcpkg toolchain does not discover static-triplet PNG/Zlib. Windows CI now installs SDL3, `zlib-ng`, and `libpng` through one `x64-windows-static` vcpkg triplet and selects that triplet in CMake for a compatible `/MT` closure, retains the depot Skia fetch, and explicitly requires C++17 for the desktop demo.

- Observation: the deterministic headless Skia scene has distinct, stable hashes for the macOS and Linux Skia archives.
  Evidence: after Linux linked successfully in hosted run `29542296307`, the scene reported `b4a6c6d43e590ffe`, while the macOS reference remains `bff7964c10eaa55f`. The test now asserts the observed Linux hash under `__linux__` and the macOS reference elsewhere, retaining an exact pixel-output guard on each supported host instead of treating cross-archive raster differences as a test failure.
