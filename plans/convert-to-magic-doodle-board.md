...
- Observation: the Windows zlib-ng and libpng depot archives use the dynamic MSVC runtime, while the published Windows Skia archive uses the static runtime.
  Evidence: after the Skia `/MT` correction, hosted Windows run `29542221102` linked Skia but reported `__imp_*` C-runtime symbols from `libpng16_static.lib` and `zlibstatic.lib`; it also exposed the desktop demo's missing C++17 target requirement. Windows CI now installs `zlib-ng` and `libpng` with vcpkg's `x64-windows-static` triplet for a compatible `/MT` closure, retains the depot Skia fetch, and explicitly requires C++17 for the desktop demo.
