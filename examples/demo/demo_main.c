...
int main(void) {
    ...
    printf("tc_demo: backend=sdl\ntc_demo: renderer=skia\ntc_demo: graphics=cpu\n");
    if (tc_backend_create(&backend_config, &backend) || tc_graphics_context_create(TC_GRAPHICS_CPU, tc_backend_get_native_window(backend), tc_backend_get_native_surface(backend), &graphics) || tc_renderer_create(TC_RENDERER_SKIA, graphics, &renderer)) {
