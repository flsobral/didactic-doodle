...
int main(int argc, char **argv) {
    ...
    { unsigned frame_limit = 0; if (argc == 3 && std::string_view(argv[1]) == "--frames") frame_limit = (unsigned)std::strtoul(argv[2], nullptr, 10); Demo demo = {nullptr, magic, renderer, {}, 0, frame_limit}; magic_doodle_board_scene_init(&demo.scene, 960, 640); BoardAppCallbacks callbacks = {sizeof(callbacks), BOARD_ABI_VERSION, nullptr, demo_event, demo_update, demo_frame, nullptr}; BoardAppConfig app_config = {sizeof(app_config), BOARD_ABI_VERSION, backend, callbacks, &demo}; if (board_app_create(&app_config, &app) != BOARD_OK || board_app_start(app) != BOARD_OK) goto cleanup; demo.app = app; if (frame_limit) std::printf("magic_doodle_board_demo: running for %u frames\n", frame_limit); else std::puts("magic_doodle_board_demo: running; close the window to exit"); result = board_app_run(app) == BOARD_OK ? 0 : 1; }
cleanup:
