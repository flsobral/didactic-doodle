...
static void on_frame(void* data, double time, double delta) { (void)time; DemoAndroid* demo = data; demo_scene_on_update(&demo->scene, delta); TcCanvas2D* canvas = tc_renderer_begin_frame(demo->renderer); if (!canvas) return; demo_scene_on_draw(&demo->scene, canvas); tc_renderer_end_frame(demo->renderer); }
...
#elif TC_ANDROID_GRAPHICS_VULKAN
    const TcGraphicsApi graphics_api = TC_GRAPHICS_VULKAN;
#else
