...
- (void)drawRect:(CGRect)rect { (void)rect; if (!graphics) return; CGColorSpaceRef colors = CGColorSpaceCreateDeviceRGB(); CGDataProviderRef data = CGDataProviderCreateWithData(NULL, graphics->pixels, (size_t)graphics->pitch * graphics->height, NULL); CGImageRef image = CGImageCreate(graphics->width, graphics->height, 8, 32, graphics->pitch, colors, kCGBitmapByteOrder32Little | kCGImageAlphaPremultipliedFirst, data, NULL, false, kCGRenderingIntentDefault); CGContextDrawImage(UIGraphicsGetCurrentContext(), self.bounds, image); CGImageRelease(image); CGDataProviderRelease(data); CGColorSpaceRelease(colors); }
