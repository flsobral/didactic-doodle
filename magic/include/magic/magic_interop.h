...
typedef struct MagicOpenGLInterop { uint32_t struct_size; uint32_t abi_version; void *context; uint32_t framebuffer; } MagicOpenGLInterop;
typedef struct MagicMetalInterop { uint32_t struct_size; uint32_t abi_version; void *device; void *command_queue; void *drawable; } MagicMetalInterop;
typedef struct MagicVulkanInterop { uint32_t struct_size; uint32_t abi_version; uint64_t image; uint64_t command_buffer; } MagicVulkanInterop;
