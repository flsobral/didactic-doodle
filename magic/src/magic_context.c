...
uint32_t magic_frame_width(const MagicFrame *frame) { return frame ? (frame->context->backend == MAGIC_BACKEND_OPENGL ? frame->opengl.width : frame->cpu.width) : 0; }
uint32_t magic_frame_height(const MagicFrame *frame) { return frame ? (frame->context->backend == MAGIC_BACKEND_OPENGL ? frame->opengl.height : frame->cpu.height) : 0; }
uint32_t magic_frame_stride(const MagicFrame *frame) { return frame ? frame->cpu.stride : 0; }
float magic_frame_scale(const MagicFrame *frame) { return frame ? (frame->context->backend == MAGIC_BACKEND_OPENGL ? frame->opengl.scale : frame->cpu.scale) : 0; }
