...
#endif
#if MAGIC_BUILD_WEB
static const MagicBackendOps magic_web_ops = { magic_web_backend_create, magic_web_backend_destroy, magic_web_backend_resize, magic_web_backend_begin_frame, magic_web_backend_end_frame };
#endif
MagicResult magic_context_create(BoardNativeSurface *surface, const MagicConfig *config, MagicContext **out_context) {
...
#endif
#if MAGIC_BUILD_WEB
        && config->backend != MAGIC_BACKEND_WEB
#endif
    ...
    if (config->backend == MAGIC_BACKEND_OPENGL) {
        ...
        free(context); return MAGIC_ERROR_UNAVAILABLE;
#endif
    } else if (config->backend == MAGIC_BACKEND_WEB) {
#if MAGIC_BUILD_WEB
        context->backend = MAGIC_BACKEND_WEB; context->ops = &magic_web_ops;
#else
        free(context); return MAGIC_ERROR_UNAVAILABLE;
#endif
...
float magic_frame_scale(const MagicFrame *frame) { return frame ? (frame->context->backend == MAGIC_BACKEND_OPENGL ? frame->opengl.scale : frame->context->backend == MAGIC_BACKEND_METAL ? frame->metal.scale : frame->context->backend == MAGIC_BACKEND_VULKAN ? frame->vulkan.scale : frame->context->backend == MAGIC_BACKEND_WEB ? frame->web.scale : frame->cpu.scale) : 0; }
