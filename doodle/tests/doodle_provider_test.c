...
int main(void) {
 ...
 DoodleRendererProvider provider = {sizeof(provider), DOODLE_ABI_VERSION, "fake", create, 0, begin, end, 0, 0, 0, 0, "test"};
 if (doodle_renderer_create(&provider, magic, &dc, &renderer) || !doodle_renderer_name(renderer) || !doodle_renderer_version(renderer) || doodle_renderer_begin_frame(renderer, frame, &canvas) || doodle_renderer_end_frame(renderer, canvas)) return 1;
