...
typedef struct BoardSurfaceOpenGLInterface { uint32_t struct_size; uint32_t abi_version; void *user_data; void *(*get_proc_address)(void *user_data, const char *name); BoardResult (*swap_buffers)(void *user_data); } BoardSurfaceOpenGLInterface;
...
typedef struct BoardSurfaceVulkanInterface { uint32_t struct_size; uint32_t abi_version; void *user_data; const char *const *(*required_instance_extensions)(void *user_data, uint32_t *count); BoardResult (*create_surface)(void *user_data, void *instance, uint64_t *out_surface); } BoardSurfaceVulkanInterface;
